...
my $filename = 'js/app.js'; # Corrected filename
...
my $found_original_checkAuthStatus = 0;

my $js_code_simulate_login = qq{
...
for my $line (@lines) {
    if ($line =~ /function simulateLogin\(\) \{/) { # Escaped the '{'
        $in_simulate_login = 1;
        $output .= $js_code_simulate_login;
    } elsif ($in_simulate_login && $line =~ /\}/) {
        $in_simulate_login = 0;
    } elsif ($line =~ /async function checkAuthStatus\(\) \{/) {
        $in_check_auth_status = 1;
        $output .= $js_code_check_auth;
    } elsif ($in_check_auth_status && $line =~ /\}/) {
        $in_check_auth_status = 0;
    } elsif (!$in_simulate_login && !$in_check_auth_status) {
        $output .= $line;
    }
}
