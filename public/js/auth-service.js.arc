...
class AuthService {
  ...
  async signup(email, password, role) {
    try {
      const securedData = this.secureLatticeData({ email, password, role });
      ...
      try {
        ...
        payload = { success: false, message: text || response.statusText };
      }
      ...
      if (!response.ok) {
        ...
        return {
          success: false,
          message: payload.message || `Signup failed (${response.status})`
        };
      }
      ...
      console.error('Signup error:', networkErr);
