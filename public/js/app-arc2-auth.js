...
function simulateLogin() {
  ...
  async function generateAndEncryptToken() {
    ...
    console.log("Generated key:", key); // Log the key here
...
async function storeEncryptedToken(encryptedToken, key) {
    try {
        const jwkKey = await exportKey(key); // Export the key
        await db.authTokens.put({ id: 1, token: encryptedToken, key: jwkKey }); // Store the exported key
