...
console.log('--- tags.js router file loaded ---'); // Check if file executes
...
router.get('/', async (req, res) => {
  ...
  console.log('*** Matched GET / handler inside tags.js ***'); // Check if handler is reached
  try {
    // Check cache first
    if (tagsCache && (Date.now() - lastCacheUpdate) < CACHE_TTL) {
      console.log('Tags route: Serving from cache.'); // Optional logging
      return res.json({
        success: true,
        tags: tagsCache,
        fromCache: true
    ...
    console.log('Tags route: Cache miss or expired. Fetching from DB.'); // Optional logging
    // Fetch all tags
    ...
    // Handle potential error from Supabase fetch
    if (error) {
      console.error('Supabase tags fetch error:', error);
      // Use the 'error' variable from destructuring above
      return res.status(500).json({
        success: false,
        message: 'Failed to retrieve tags from database.',
        error: error.message // Corrected variable name
      });
    }

    // --- Success Case ---
    // If fetching was successful (no error)
    console.log(`Tags route: Fetched ${tags.length} tags from DB. Updating cache.`); // Optional logging

    // Update cache
    tagsCache = tags;
    lastCacheUpdate = Date.now();

    // Send successful response with fetched tags
    return res.json({
      success: true,
      tags // Send the fetched tags
      // Optionally add fromCache: false here if needed by frontend
    });

  } catch (err) { // --- Catch Block for Unexpected Errors ---
    // Catch any other errors (e.g., issues with Date.now(), logic errors)
    console.error('Unexpected error in GET /tags route:', err);
    return res.status(500).json({
      success: false,
      message: 'An unexpected server error occurred.',
      error: err.message // Use the 'err' variable from the catch block
    });
  }
