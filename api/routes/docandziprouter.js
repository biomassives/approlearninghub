...
async function generateFileContent(fileConfig) {
  switch (fileConfig.format) {
    case 'csv':
      ...
      break;
    case 'pdf':
        const { jsPDF } = require('jspdf-node');
...
router.get('/download-data', async (req, res) => {
  ...
  try {
    ...
    const supabaseUrl = process.env.SUPABASE_URL; //  Use from your .env
    const supabaseKey = process.env.SUPABASE_KEY; //  Use from your .env
    ...
    for (const fileConfig of contents) {
      try {
        ...
        if (fileConfig.type === 'static') {
          ...
          const connectionString = process.env.POSTGRES_CONNECTION_STRING; // Use from .env
          if (!connectionString) {
